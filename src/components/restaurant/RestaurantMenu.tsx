...
export default function RestaurantMenu({ sections, restaurantId, restaurantName }: RestaurantMenuProps) {
    ...
    const handleConflictConfirm = () => {
        ...
        replaceCart(
            { foodId: conflictItem.id, name: conflictItem.name, price: conflictItem.price, image: conflictItem.imageUrl ?? "" },
            restaurantId,
            restaurantName,
        );
        ...
        <>
            ...
            <div className="flex flex-col gap-10 py-6">
                {sections.map(({ category, items }) => (
                    ...
                    >
                        <h2 className="text-xl font-bold text-stone-900 mb-4">{category.name}</h2>
