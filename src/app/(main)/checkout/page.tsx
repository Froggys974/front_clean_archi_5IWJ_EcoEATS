...
type Address = { street: string; city: string; zip: string; instructions?: string };

export default function CheckoutPage() {
    const { isAuthenticated, isLoading } = useAuth();
    const { items, cartId, restaurantName, subtotal, clearCart, isLoading: isCartLoading } = useCart();
    const { post } = useApi();
    const router = useRouter();

    const [step, setStep] = useState<Step>("address");
    const [address, setAddress] = useState<Address>({ street: "", city: "", zip: "", instructions: "" });
    ...
    useEffect(() => {
        ...
        if (!isLoading && !isCartLoading && isAuthenticated && items.length === 0) router.push("/restaurants");
    }, [isLoading, isAuthenticated, isCartLoading, items.length, router]);

    if (isLoading || isCartLoading || !isAuthenticated || items.length === 0) {
        return (
    ...
    const handleAddressNext = () => {
        if (validateAddress()) { setErrors({}); setStep("payment"); }
    };
    ...
    const handlePaymentSubmit = async () => {
        ...
        try {
            const order = await post<OrderDto>('/orders/checkout', {
                cartId: cartId!,
                deliveryStreet: address.street,
                deliveryCity: address.city,
                deliveryPostalCode: address.zip,
                deliveryCountry: "France",
            });
            await clearCart();
            router.push(`/orders/${order.id}`);
        } catch {
            setErrors({ submit: "Le paiement a échoué. Veuillez réessayer." });
        } finally {
                            ...
                            <div className="flex flex-col items-center gap-1.5">
                                <div
                                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                                        i < stepIndex ? "bg-accent text-white" : i === stepIndex ? "text-white" : "bg-stone-200 text-stone-400"
                                    }`}
                    ...
                    <div className="lg:col-span-2">
                            ...
                            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 flex flex-col gap-5">
                                ...
                                <Field label="Adresse" error={errors.street}>
                                    <input type="text" value={address.street}
                                        ...
                                        placeholder="12 rue de la Paix" className={inputClass(!!errors.street)} />
                                </Field>
                                <div className="grid grid-cols-2 gap-4">
                                    <Field label="Ville" error={errors.city}>
                                        <input type="text" value={address.city}
                                            onChange={(e) => setAddress({ ...address, city: e.target.value })}
                                            placeholder="Paris" className={inputClass(!!errors.city)} />
                                    </Field>
                                    <Field label="Code postal" error={errors.zip}>
                                        <input type="text" value={address.zip}
                                            onChange={(e) => setAddress({ ...address, zip: e.target.value })}
                                            placeholder="75001" className={inputClass(!!errors.zip)} />
                                    </Field>
                                </div>
                                <Field label="Instructions (optionnel)" error="">
                                    <textarea value={address.instructions}
                                        onChange={(e) => setAddress({ ...address, instructions: e.target.value })}
                                        placeholder="Digicode, étage…" rows={3}
                                        className={`${inputClass(false)} resize-none`} />
                                </Field>
                                <button onClick={handleAddressNext}
                                    className="cursor-pointer w-full py-3.5 rounded-xl font-bold text-white text-base transition-all hover:shadow-lg"
                        ...
                        {step === "payment" && (
                            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 flex flex-col gap-5">
                                ...
                                <p className="text-sm text-stone-400 -mt-3">Aucune donnée réelle n&apos;est transmise.</p>
                                ...
                                <Field label="Titulaire de la carte" error={errors.name}>
                                    <input type="text" value={card.name}
                                        ...
                                        placeholder="Jean Dupont" className={inputClass(!!errors.name)} />
                                </Field>
                                ...
                                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                                    <button onClick={() => { setErrors({}); setStep("address"); }}
                                    ...
                                    <button onClick={handlePaymentSubmit} disabled={isSubmitting}
                                        className="cursor-pointer flex-2 py-3.5 rounded-xl font-bold text-white text-base transition-all hover:shadow-lg disabled:opacity-70"
                                        style={{ background: "linear-gradient(to right, var(--primary), var(--accent))" }}>
                                        {isSubmitting ? (
                            ...
                            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
                                {items.map((item) => (
                                    <div key={item.foodId} className="flex items-center justify-between gap-2 text-sm">
                                        ...
                                        <span className="text-stone-700 font-medium shrink-0">{(item.price * item.quantity).toFixed(2)} €</span>
                                    </div>
                                ))}
                            </div>
...
function inputClass(hasError: boolean) {
    return `w-full px-4 py-3 rounded-xl border text-sm outline-none transition-colors ${
        hasError ? "border-red-300 focus:border-red-400 bg-red-50" : "border-stone-200 focus:border-accent bg-white"
    }`;
}
